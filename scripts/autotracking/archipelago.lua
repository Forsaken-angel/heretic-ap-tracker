...
function onClear(slot_data)
    ...
    if slot_data['check_sanity'] then
        local obj = Tracker:FindObjectForCode("checks")
        local stage = slot_data['check_sanity']
        if (stage >=2) then
            stage = 2
        end
        if obj then
    ...
    if slot_data['pro'] then
        local obj = Tracker:FindObjectForCode("pro_mode")
    ...
    if slot_data['episode5'] then
        local obj = Tracker:FindObjectForCode("ep5")
        local stage = slot_data['episode5']
        if (stage >=2) then
            stage = 2
        end
        if obj then
    ...
    if PLAYER_ID>-1 then
        local eventId="heretic_events_"..TEAM_NUMBER.."_"..PLAYER_ID
